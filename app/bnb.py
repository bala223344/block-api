    ...
    if tx_count is None or total_current_tx > tx_count:
        ...
        if email_id is not None:
            message = Mail(
                ...
                html_content= '<h3> You got a new transaction on your BNB address </h3><strong>Address:</strong> ' + str(address) +'')
...
def EthDataSyncing(address,symbol,type_id):
    ret = BNB_balance
    response_user_token = requests.get(url=ret)
    response = response_user_token.json()       
    doc = BNB_transactions
    response_user = requests.get(url=doc)
    res = response_user.json()       
    ...
    for transaction in transactions:
        ...
        if send_amount == "0":
            ...
            docc=BNB_transactions.replace("{{hash}}",''+tx_id+'')
