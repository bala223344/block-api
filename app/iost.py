...
def iost_data(address,symbol,type_id):
    ret=IOST_balance.replace("{{address}}",''+address+'')
    response_user_token = requests.get(url=ret)
    response = response_user_token.json()       
    
    doc=IOST_transactions.replace("{{address}}",''+address+'')
    ...
    for transaction in transactions:
        ...
        if contractAddress == "0xfa1a856cfa3409cfa145fa4e20eb270df3eb21ab":
...
def iost_notification(address,symbol,type_id):
    doc=IOST_transactions.replace("{{address}}",''+address+'')
    response_user = requests.get(url=doc)
    res = response_user.json()  
    transactions=res['result']
    ...
    if tx_count is None or total_current_tx > tx_count:
        ...
        if email_id is not None:
            message = Mail(
                ...
                html_content= '<h3> You got a new transaction on your IOST address </h3><strong>Address:</strong> ' + str(address) +'')
