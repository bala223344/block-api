...
def rep_notification(address,symbol,type_id):    
    doc=REP_transactions.replace("{{address}}",''+address+'')
    response_user = requests.get(url=doc)
    res = response_user.json()  
    transactions=res['result']
    ...
    print("total_current_tx",total_current_tx)
    if tx_count is None or total_current_tx > tx_count:
        ...
        if email_id is not None:
            message = Mail(
                ...
                html_content= '<h3> You got a new transaction on your REP address </h3><strong>Address:</strong> ' + str(address) +'')
