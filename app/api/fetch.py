...
def main():
    ...
    if symbol == "BTC":
        currency = btc_data(address,symbol)
        return currency
    if symbol == "ETH":
        currency = eth_data(address,symbol)
        return currency
    if symbol == "LTC":
        currency = ltc_data(address,symbol)
        return currency
    if symbol == "BTC_CASH":
        currency = btc_cash_data(address,symbol)
        return currency
    if symbol == "BINANCE_COIN":
        currency = b_chain_data(address,symbol)
        return currency
    if symbol == "BTC_SV":
        currency = bitcoin_svs_data(address,symbol)
        return currency
    if symbol == "TETHER":
        currency = tether_data(address,symbol)
        return currency

    if symbol == "TRON":
        currency = tron_data(address,symbol)
        return currency

    if symbol == "UNUS_SED_LEO":
        currency = unus_sed_leo_data(address,symbol)
        return currency

    if symbol == "IOTA":
        currency = iota_data(address,symbol)
        return currency

    if symbol == "Z_CASH":
        currency = zcash_data(address,symbol)
        return currency
    
    if symbol == "ONT":
        currency = ont_data(address,symbol)
        return currency

    if symbol == "XTZ":
        currency = xtz_data(address,symbol)
        return currency

    if symbol == "BTC_GOLD":
        currency = btc_gold_data(address,symbol)
        return currency
    #Nt done
    if symbol == "QTUM":
        currency = qtum_data(address,symbol)
        return currency

    if symbol == "MKR":
        currency = mkr_data(address,symbol)
        return currency

    if symbol == "VET":
        currency = vet_data(address,symbol)
        return currency

    if symbol == "CRO":
        currency = cro_data(address,symbol)
        return currency

    if symbol == "BAT":
        currency = bat_data(address,symbol)
        return currency

    if symbol == "USDC":
        currency = usdc_data(address,symbol)
        return currency

    if symbol == "BTT":
        currency = btt_data(address,symbol)
        return currency
...
def local_transaction(address):
    docs = mongo.db.balance.find({"address":address})
