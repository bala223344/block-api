...
from app.config import MAID_balance,MAID_transactions
...
def maid_data(address,symbol,type_id):
    ret=MAID_balance.replace("{{address}}",''+address+'')
    response_user_token = requests.get(url=ret)
    response = response_user_token.json()       
    
    doc=MAID_transactions.replace("{{address}}",''+address+'')
    ...
    for transaction in transactions:
        ...
        if contractAddress == "0x6fe2d586354bfb3bebd38f9e772ae965a1c7681d":
...
def maid_notification(address,symbol,type_id):
    doc=MAID_transactions.replace("{{address}}",''+address+'')
    response_user = requests.get(url=doc)
    res = response_user.json()  
    transactions=res['result']
    ...
    if tx_count is None or total_current_tx > tx_count:
        ...
        if email_id is not None:
            message = Mail(
                ...
                html_content= '<h3> You got a new transaction on your MAID address </h3><strong>Address:</strong> ' + str(address) +'')
