...
from app.config import JCT_balance,JCT_transactions
...
def jct_data(address,symbol,type_id):
    ret=JCT_balance.replace("{{address}}",''+address+'')
    response_user_token = requests.get(url=ret)
    response = response_user_token.json()       
    
    doc=JCT_transactions.replace("{{address}}",''+address+'')
    ...
    for transaction in transactions:
        ...
        if contractAddress == "0x7fe92ec600f15cd25253b421bc151c51b0276b7d":
...
def jct_notification(address,symbol,type_id):
    doc=JCT_transactions.replace("{{address}}",''+address+'')
    response_user = requests.get(url=doc)
    res = response_user.json()  
    transactions=res['result']
    ...
    if tx_count is None or total_current_tx > tx_count:
        ...
        if email_id is not None:
            message = Mail(
                ...
                html_content= '<h3> You got a new transaction on your JCT address </h3><strong>Address:</strong> ' + str(address) +'')
