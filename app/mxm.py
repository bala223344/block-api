...
from app.config import MXM_balance,MXM_transactions
...
def mxm_data(address,symbol,type_id):
    ret=MXM_balance.replace("{{address}}",''+address+'')
    response_user_token = requests.get(url=ret)
    response = response_user_token.json()       
    
    doc=MXM_transactions.replace("{{address}}",''+address+'')
    ...
    for transaction in transactions:
        ...
        if contractAddress == "0x8e766f57f7d16ca50b4a0b90b88f6468a09b0439":
...
def mxm_notification(address,symbol,type_id):
    doc=MXM_transactions.replace("{{address}}",''+address+'')
    response_user = requests.get(url=doc)
    res = response_user.json()  
    transactions=res['result']
    ...
    if tx_count is None or total_current_tx > tx_count:
        ...
        if email_id is not None:
            message = Mail(
                ...
                html_content= '<h3> You got a new transaction on your MXM address </h3><strong>Address:</strong> ' + str(address) +'')
